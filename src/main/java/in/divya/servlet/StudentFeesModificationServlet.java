...
/**
 * Servlet implementation class StudentFeesModificationServlet
 */
@WebServlet("/StudentFeesModificationServlet")
public class StudentFeesModificationServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public StudentFeesModificationServlet() {
		super();
	}
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			/**
			 * Step 2: Call Service => Update student Fees details
			 */
			...
			boolean isUpdatedFees = studentFeesDetailService.updateFees(fees);

			/**
			 * Step 3: Decide to which page we should redirect ?
			 */
			if (isUpdatedFees) {
				String message = "SUCCESSFULLY FEES UPDATED";
				response.sendRedirect("StudentFeesAdd.jsp?infoMessage=" + message);

			} else {
				throw new InValidCredentialsException("Cannot update Fees");
